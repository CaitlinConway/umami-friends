...
export function gameActionHelper(socket, io, action, roomCode, userName) {
    ...
    switch (action.actionType) {
        ...
        case "startTurn":
            ...
            sweetIngredients &&
                currentPlayer.board.candy.push(...sweetIngredients);
            currentPlayer.energy = 1;
            break;
        case "drawCard":
        ...
        case "moveCard":
            ...
            if (![0, 1].includes(action.data.zone.to.user)) {
                ...
                if (action.data.zone.to.zoneName === "board") {
                    const cardsDetails = action.data.cards.map(
                        each => allCards[each]
                    );
                    ...
                    gameState["users"][
                        action.data.zone.to.user
                    ].board.candy.push(candyCards);
                    gameState["users"][
                        action.data.zone.to.user
                    ].board.ingredients.push(ingredientCards);
                } else {
