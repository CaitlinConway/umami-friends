...
const Game = (props) => {
	...
	const [selectedHandValues, setSelectedHandValues] = useState(initialHandValue);
	...
	const endTurn = () => {
		...
		socket.emit('gameAction', { actionType: 'endTurn', actionData: { discardCards } }, roomCode, userName);
		socket.emit('gameAction', { actionType: 'startTurn' }, roomCode, userName);
	};
	const buyCard = () => {
		socket.emit(
	...
	const cardClick = (clickedCard, hand = false, discard = false) => {
		if (hand && !discard) {
			if (selectedHand?.includes(clickedCard)) {
				...
				setSelectedHand(selectedHand.filter((card) => card.name !== clickedCard.name));
			} else {
			...
			if (discardCards?.includes(clickedCard)) {
				// If selected, remove from selected cards
				setDiscardCards(discardCards.filter((card) => card.name !== clickedCard.name));
			} else {
				// If not selected, add to selected cards
	...
	const buyEnergy = () => {
		...
		socket.emit('gameAction', { actionType: 'buyEnergy', actionData: { selectedHand } }, roomCode, userName);
	};
	useEffect(() => {
		let sweets = 0;
	...
	return (
		<div className='gameBackground'>
          ...
          <CardStack ingredient={false}></CardStack>
				...
				<div className='buttonsAndMessages'>
					<div className='playerHandContainer buttonGroup'>
						<button className='drawButton' onClick={drawCard}>
							DRAW 3 CARDS
						</button>
						{/* TODO: Add onClick action */}
						<button className='ingredientButton'>PLACE INGREDIENT</button>
						<button
							className='actionButton'
							// TODO: Also disable if action cannot be taken
							disabled={playerRoles[props.role]?.userActionIsPassive}
							onClick={() => playerRoles[props.role]?.userAction(socket, roomCode)}
						>
							TAKE ROLE ACTION
						</button>
						<button className='buyEnergyButton' disabled={!canBuyEnergy} onClick={buyEnergy}>
							BUY ENERGY
						</button>
					</div>
					<div className='playerHandContainer messageContainer'>
						<Messages user={userName} opponent={opponent?.name}></Messages>
					</div>
				</div>
			</div>
		</div>
